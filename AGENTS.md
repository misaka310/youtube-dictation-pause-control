...
- 拡張機能の読み込み対象は`extension/`とする。変更時は共有`browser-extension-update-delivery`を適用し、`scripts/reload-extension.ps1`で自己再読み込みと`expectedVersion`／`expectedBuildId`、既存サーバー連携の反映確認まで行う。
