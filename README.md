...
Windowsで音声入力中だけYouTubeを自動一時停止するローカル補助ツールです。Typeless、Wispr Flow、ChatGPT Local Voice Bridgeの入力操作を通知領域常駐EXEで検知し、BraveのYouTubeタブだけを拡張機能から制御します。
...
```text
Typeless / Wispr Flow / Local Voice Bridge input
...
- Typeless / Wispr Flowのトグル操作とLocal Voice Bridgeの長押し操作を監視

### 4. 動作確認

1. BraveでYouTube動画を再生します。
2. Wispr Flowは`Ctrl + ]`、Typelessは右Ctrlを先に押したまま右Shiftを押して開始します。どちらも1回押すごとのトグルです。
3. ChatGPT Local Voice Bridgeは右Ctrlを押したまま、右Shift左の`＼ / _`キーを押し続けて開始します。
4. 動画が一時停止することを確認します。
...
Typeless / Wispr Flowを停止し、Local Voice Bridgeの2キーを離してから`Ctrl + Alt + R`を押すか、通知領域の`Reset dictation state`を選びます。内部のTypeless・Wispr Flow・Local Voice Bridge・集約状態とdebounce時刻をinactiveへ戻し、HTTP Bridgeにもinactiveを送ります。
...
- Typeless / Wispr Flow本体の録音状態は直接取得せず、ホットキー押下を状態トグルとして扱います。Local Voice Bridgeは右Ctrlと右Shift左の`＼ / _`キーを押している間だけactiveとして扱います。
