...
| TypelessとWispr Flowの同時使用 | 2つの真偽値をOR集約 | 最初の開始で停止し、両方終了まで再生しない | 自動テストなし | 未実施 | 両方を終了して`Ctrl + Alt + R` | 各アプリの実録音状態とは同期しない |
...
| `Ctrl + Alt + R`状態同期リセット | AHKのTypeless・Wispr Flow・集約状態をfalse、debounce時刻を0へ戻し、Bridgeへinactiveを送信 | 次の通常ホットキーが開始扱いに戻る。所有権とsessionIdが一致すれば停止中動画の再生も試みる | 自動契約テスト済み: 11ケース、AHK構文確認済み | 未実施 | 入力アプリを必ず停止してから押す | 音声入力アプリ本体の録音は停止しない。録音中に押すと再びずれる |
