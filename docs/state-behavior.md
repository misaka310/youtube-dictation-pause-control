...
| Local Voice Bridgeのみ | 音声入力側が録音開始・終了を`source=local-voice-bridge`として直接通知 | 録音中に停止し、終了すると条件付き再生 | 自動テスト済み: 通知クライアント / API集約 | 未実施 | 2キーを離し、両アプリの起動状態と`/health`を確認 | YouTube側はキーを再監視しない。通知先が不在でも録音自体は継続する |
| 複数入力の同時使用 | HTTP Bridgeの`activeSources`をOR集約 | 最初の開始で停止し、すべての入力元が終了するまで再生しない | 自動テスト済み: API 18ケース | 未実施 | すべての入力を終了して`Ctrl + Alt + R` | Typeless / Wispr Flowの実録音状態とは同期しない |
| 一部だけ終了 | 終了した入力元だけを`activeSources`から削除し、残りがあれば集約activeを維持 | 動画を再生しない | 自動テスト済み: API集約 | 未実施 | 残りの入力も終了する | トグル式入力はホットキー取りこぼしで判定が逆転し得る |
| すべて終了 | `activeSources`が空になり集約active=false | 同一sessionで所有していた動画だけ再生 | 自動テスト済み: API集約 | 未実施 | 再生拒否時は手動再生 | トグル式入力の内部フラグがずれると終了しない |
...
| Typeless / Wispr Flowホットキー取りこぼし | AHK内部フラグが実録音状態とずれる | 実装はずれを自動検出できない | 自動テストなし | 未実施 | 両アプリを終了し、`Ctrl + Alt + R`を押す | Local Voice Bridgeは実録音開始・終了を直接通知するため、この制限の対象外 |
| Typeless / Wispr Flowがホットキー以外で終了 | AHKは終了を検知せず`source=ahk`がactiveのまま | 動画は停止維持され得る | 自動テストなし | 未実施 | 入力アプリが停止済みであることを確認し、`Ctrl + Alt + R` | Local Voice Bridgeは録音終了イベントを直接通知する |
| Typeless / Wispr Flow実録音状態とAHK内部状態のずれ | `source=ahk`へ誤ったactive/inactiveが送られる | 自動補正しない | 自動テストなし | 未実施 | 全入力を終了して`Ctrl + Alt + R`、必要なら動画を手動復旧 | Local Voice Bridgeの直接通知とは独立 |
| `Ctrl + Alt + R`状態同期リセット | AHKのTypeless・Wispr Flow状態とdebounce時刻を初期化し、`POST /state`の`source=*`, `active=false`で全`activeSources`を解除。sessionIdは維持 | 集約状態がinactiveになり、同じsessionで所有している動画の再開を試みる | 自動契約テスト済み: 23ケース、API集約確認済み、AHK構文確認済み | 未実施 | 入力アプリを停止し、Local Voice Bridgeの2キーを離してから押す | 音声入力アプリ本体の録音は停止しない。録音中に押すと再通知でactiveへ戻る |
...
| `POST /reset` | `activeSources`を空にし、`active=false, sessionId=0`へ初期化 | APIは初期化成功。Content Script側に旧session所有権があればID不一致で再生せず破棄 | 自動テスト済み: API resetと次session=1 | 未実施 | AHK状態もずれている場合は、入力を終了して`Ctrl + Alt + R`を使用 | `/reset`だけではAHK内部フラグを初期化しない |
