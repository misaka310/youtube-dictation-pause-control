...
### ① `scripts/windows/setup-autostart.bat` の動作テスト
- **手順**:
  1. `scripts/windows/setup-autostart.bat` をダブルクリックして起動します。
...
### ② `scripts/windows/remove-autostart.bat` の動作テスト
- **手順**:
  1. `scripts/windows/remove-autostart.bat` をダブルクリックして起動します。
...
- **手順**:
  ...
  2. このツールが起動した AHK（タスクトレイの「H」アイコン）および Node.js サーバーが正常に終了し、プロンプトに `[SUCCESS] Stop command completed.` と表示されることを確認します。
  3. 他の AutoHotkey スクリプトを起動している場合、それらが停止されていないことを確認します。

---

## 4. 完了前チェック

- [ ] Wispr Flowの開始・終了で停止・再生する
- [ ] Typelessの開始・終了で停止・再生する
- [ ] 最初から停止中の動画を再生しない
- [ ] 入力中のPause Guardが動作する
- [ ] YouTube SPA遷移後の動画を停止する
- [ ] TypelessとWispr Flowを交互に使っても状態が混ざらない
- [ ] 400ms以内の短い入力でも停止・再生する
- [ ] Node.jsサーバー停止時に動画を勝手に再生しない
- [ ] 拡張機能停止時に動画を勝手に再生しない
- [ ] AHK停止時に動画を勝手に再生しない
- [ ] `/reset`後に状態を安全に初期化できる
- [ ] `stop.bat`後にこのツールのAHKプロセスだけが停止する
- [ ] ブラウザを閉じて再度開いた後も、入力開始で停止する

各項目で、操作・結果・ログ・失敗時の復旧手順を記録します。
